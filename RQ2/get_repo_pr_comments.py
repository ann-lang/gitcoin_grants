...
def obtain_pr_comments(repo, start_date, end_date):
	i = 1
	idx = 0
	pr_comments = []
	url = 'https://api.github.com/repos/' + repo[19:] + '/pulls/comments?since=%s&page=%s&per_page=100'%(start_date, i)
	print(url)
		...
		while len(response_dict) > 0:
			...
			for c in response_dict:
				...
				if created_at < datetime.strptime(end_date, "%Y-%m-%dT%H:%M:%SZ"):
					pr_comments.append(created_at.strftime("%Y-%m-%dT%H:%M:%SZ"))
					#print(pr_comments)
				else:
					flag = True
					break
...
for row in tqdm(csv_reader):
	...
	for repo in repos:
		repo = repo.strip()
		try:
			pr_comments = obtain_pr_comments(repo, start_date, end_date)
			print('len_pr_comments: %s'%len(pr_comments))
			if row[0] not in repo_pr_comments:
				repo_pr_comments[row[0]] = pr_comments
			else:
				repo_pr_comments[row[0]] += pr_comments
		except:
			repo_pr_comments[row[0]] = []
file1.close()

file2 = open('top500_pr_comments.csv', 'w')
csv_writer = csv.writer(file2)
csv_writer.writerow(['pk', 'pr_comments'])
for k in repo_pr_comments:
	csv_writer.writerow([k, ';'.join(item for item in repo_pr_comments[k])])
file2.close()
